...
def test_normalize(mock_get_table_columns):
    ...
    SELECT ROUND(ROUND((column_to_normalize - min_value),2) / ROUND((max_value - min_value),2),2) AS normalized, node_table.gid, node_table.geom
    FROM node_table, (SELECT MIN(column_to_normalize) AS min_value, MAX(column_to_normalize) AS max_value FROM node_table) AS min_max;
    """
